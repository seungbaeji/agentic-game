...
Dialogue는 `DialoguePhase.END`에 도달하기 전까지 active session으로 유지됩니다. 그래서 다음처럼 scenario 이름을 다시 말하지 않아도 이어집니다.
...
`떠나기`처럼 `END`로 이동하는 action을 선택하면 parent의 `current_subgraph`가 비워집니다.

반대로 dialogue 중에 `메뉴 보여줘` 같은 pause 입력을 하면 capability 안내를 보여주지만 dialogue session은 유지됩니다. `그만하고 탐험할래`처럼 다른 scenario를 명시하면 dialogue가 terminal phase에 도달하지 않았더라도 active session은 exploration으로 전환됩니다.
