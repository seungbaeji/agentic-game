...
같은 입력이면 항상 같은 출력을 반환하는 순수 계산입니다.

예:

```text
PlayerState + hp_change -> updated PlayerState
```

외부 store, LLM, random, time을 읽으면 pure/deterministic이 아닙니다.

### Stateless

객체나 프로세스가 호출 사이의 상태를 자기 메모리에 보관하지 않는 성질입니다.

Stateless가 반드시 "같은 입력이면 같은 출력"을 뜻하지는 않습니다. 외부 store를 읽으면 같은 입력이어도 결과가 달라질 수 있습니다.

### Stateful

이전 호출의 상태가 다음 호출에 영향을 주는 성질입니다. 상태는 메모리, DB, store, session 어디에 있어도 됩니다.

### State Scope

상태가 누구에게 속하는지 정하는 경계입니다.
...
```text
전투 결과로 player HP/EXP 갱신
제작 결과로 inventory 갱신
```

### Query

상태를 읽기만 하는 요청입니다. store를 읽으므로 pure는 아니지만 side effect는 없어야 합니다.

### Repository

application usecase가 domain state를 읽고 쓰기 위해 사용하는 추상화입니다. 실제 저장소 구현은 outbound adapter에 둡니다.

### Unit of Work

하나의 command 처리 중 여러 state 변경을 하나의 transaction으로 묶는 경계입니다.

### Idempotency

같은 command가 재시도되어도 상태 변경이 중복 적용되지 않는 성질입니다.

### Concurrency

여러 요청이 동시에 같은 state를 읽고 쓰면서 발생하는 문제입니다. API나 shared store 환경에서는 version, lock, transaction, append-safe key 같은 전략이 필요합니다.
