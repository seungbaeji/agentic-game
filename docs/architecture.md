...
- inventory 같은 게임 상태 모델과 순수 변경 규칙
...
`game_state.py`는 `StorePort` 위에 놓인 작은 repository입니다. 현재는 craft 성공 결과를 player inventory에 저장하고, skill_training 결과를 skill book에 저장합니다.
...
`StorePort`는 낮은 수준의 put/get만 제공하고, inventory와 skill book을 어떻게 읽고 갱신할지는 `GameStateRepository`가 담당합니다.
...
현재 실제 tool-backed 시나리오는 battle과 craft뿐입니다. craft는 성공 시 `GameStateRepository`를 통해 inventory를 갱신합니다. skill_training은 아직 tool-backed는 아니지만 deterministic node에서 `GameStateRepository`를 통해 skill book을 갱신합니다. exploration, quest, trade, dialogue는 아직 `tools/` 계층을 거치지 않고 각 scenario execute/response node에서 deterministic response를 반환합니다.
...
| 종류 | 대상 | 특징 |
| --- | --- | --- |
| tool-backed execute node | battle, craft | `engine/tool_runner.py`를 통해 LangChain tool과 application usecase를 호출하고 payload를 store에 저장한다. craft는 inventory도 갱신한다. |
