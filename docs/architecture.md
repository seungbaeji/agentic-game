...
- player, inventory, skill book 같은 게임 상태 모델과 순수 변경 규칙
...
`game_state.py`는 `StorePort` 위에 놓인 작은 repository입니다. 현재는 battle 결과를 player state에 저장하고, craft 성공 결과를 player inventory에 저장하고, exploration 발견 상태를 world state에 저장하고, quest 진행 상태를 quest log에 저장하고, skill_training 결과를 skill book에 저장합니다.
...
`StorePort`는 낮은 수준의 put/get만 제공하고, player, inventory, quest log, skill book, world state를 어떻게 읽고 갱신할지는 `GameStateRepository`가 담당합니다.
...
tool의 입력에는 usecase, random adapter, 필요한 game state repository가 injected argument로 들어갑니다. tool은 usecase 결과를 LLM/UI/raw 용 payload로 projection하고 `ToolResult`를 반환합니다.

현재 실제 tool-backed 시나리오는 battle과 craft뿐입니다. battle은 `GameStateRepository`를 통해 player HP/EXP를 갱신하고, craft는 성공 시 inventory를 갱신합니다. exploration은 deterministic node에서 world state를 갱신하고, trade는 player gold와 inventory를 갱신하고, quest는 quest log와 player reward를 갱신하고, skill_training은 deterministic node에서 skill book을 갱신합니다. dialogue는 아직 `tools/` 계층을 거치지 않고 각 scenario execute/response node에서 deterministic response를 반환합니다.

### scenarios

`scenarios/`는 어떤 게임 시나리오가 있고, 사용자 입력이 어떤 scenario/event로 해석되는지 설명합니다.
...
현재 `tool_runner.py`가 알고 있는 concrete tool binding은 battle/craft입니다.
...
다른 시나리오는 아직 이 runner를 사용하지 않습니다. 이 경계는 의도적으로 좁게 유지되어 있습니다. scenario flow와 graph shape 일반화가 먼저이며, tool/usecase/game state 일반화는 다음 확장 지점입니다.
...
| 종류 | 대상 | 특징 |
| --- | --- | --- |
| tool-backed execute node | battle, craft | `engine/tool_runner.py`를 통해 LangChain tool과 application usecase를 호출하고 payload를 store에 저장한다. battle은 player, craft는 inventory도 갱신한다. |
| deterministic execute/response node | exploration, quest, trade, dialogue, skill_training | lightweight sample 로직으로 response를 만든다. exploration은 world, trade는 player/inventory, quest는 quest log/player reward, skill_training은 skill book 저장까지 수행한다. |
...
3. `engine/tool_runner.py`에 event-to-tool-input 변환과 payload persistence 호출을 추가한다.
4. scenario execute node가 직접 결과를 만들지 않고 tool runner에 위임하게 바꾼다.
5. `bootstrap.py`, `agent/graph/parent.py`, `scenarios/registry.py`에서 hydrated tool/usecase dependency를 주입한다.
