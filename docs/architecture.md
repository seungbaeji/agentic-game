...
tool의 입력에는 usecase, random adapter, 필요한 state repository가 injected argument로 들어갑니다. tool은 usecase 결과를 LLM/UI/raw 용 payload로 변환하고 `ToolResult`를 반환합니다.

`types.py`의 `ToolResult`는 tool 경계에서만 쓰는 입출력 객체입니다. 프로젝트 전체의 DTO를 한곳에 모으지 않고, `agent/state.py`, `agent/decisions.py`, `flow/models.py`, `tools/types.py`처럼 각 경계 가까이에 둡니다. 이렇게 해야 객체가 어느 계층의 언어인지 바로 보입니다.
...
LLM은 scenario 입력을 더 자연스럽게 해석할 때 사용합니다. 명시적인 행동 입력은 `intent.py`의 deterministic fast-path로 처리하고, 후속 질문이나 애매한 입력은 scenario decision prompt가 `action`, `question`, `clarify`, `smalltalk` 같은 의도로 분류합니다. 이때 `action`만 `flow/` 전이를 통과합니다. `question`과 `smalltalk`은 phase를 변경하지 않고 현재 scenario 맥락에서 응답합니다.

중요한 경계는 다음과 같습니다.

- LLM은 사용자 입력 이해, event 후보 선택, 후속 질문 응답, 표현/요약을 맡습니다.
- `flow/`는 phase/event 전이를 검증합니다.
- usecase와 tool은 실제 상태 변경을 수행합니다.
- store는 확정된 결과와 scenario state를 저장합니다.
...
CraftEvent  -> category + CraftPlan -> craft_item_tool
...
CraftEvent
  -> CraftCategory
  -> CraftPlan(item_name/display_name/requested_effect)
  -> craft_item_tool
  -> craft_item_and_store_reward
