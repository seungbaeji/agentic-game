...
현재 실제 tool-backed 시나리오는 battle과 craft뿐입니다. battle은 `GameStateRepository`를 통해 player HP/EXP를 갱신하고, craft는 성공 시 inventory를 갱신합니다. skill_training은 아직 tool-backed는 아니지만 deterministic node에서 skill book을 갱신합니다. exploration, quest, trade, dialogue는 아직 `tools/` 계층을 거치지 않고 각 scenario execute/response node에서 deterministic response를 반환합니다.
...
| deterministic execute/response node | exploration, quest, trade, dialogue, skill_training | lightweight sample 로직으로 response를 만든다. skill_training은 skill book 저장까지 수행한다. |
