...
`game_state.py`는 `StorePort` 위에 놓인 작은 repository입니다. 현재는 battle 결과를 player state에 저장하고, craft 성공 결과를 player inventory에 저장하고, quest 진행 상태를 quest log에 저장하고, skill_training 결과를 skill book에 저장합니다.
...
`StorePort`는 낮은 수준의 put/get만 제공하고, player, inventory, quest log, skill book을 어떻게 읽고 갱신할지는 `GameStateRepository`가 담당합니다.
...
현재 실제 tool-backed 시나리오는 battle과 craft뿐입니다. battle은 `GameStateRepository`를 통해 player HP/EXP를 갱신하고, craft는 성공 시 inventory를 갱신합니다. trade는 deterministic node에서 player gold와 inventory를 갱신하고, quest는 quest log와 player reward를 갱신하고, skill_training은 deterministic node에서 skill book을 갱신합니다. exploration, dialogue는 아직 `tools/` 계층을 거치지 않고 각 scenario execute/response node에서 deterministic response를 반환합니다.
...
| deterministic execute/response node | exploration, quest, trade, dialogue, skill_training | lightweight sample 로직으로 response를 만든다. trade는 player/inventory, quest는 quest log/player reward, skill_training은 skill book 저장까지 수행한다. |
