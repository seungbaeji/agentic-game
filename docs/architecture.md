...
`game_state.py`는 `StorePort` 위에 놓인 작은 repository입니다. 현재는 craft 성공 결과를 player inventory에 저장합니다.

```text
game/inventory/latest -> InventoryState
```

`StorePort`는 낮은 수준의 put/get만 제공하고, inventory를 어떻게 읽고 갱신할지는 `GameStateRepository`가 담당합니다.
...
현재 실제 tool-backed 시나리오는 battle과 craft뿐입니다. craft는 성공 시 `GameStateRepository`를 통해 inventory를 갱신합니다. exploration, quest, trade, dialogue, skill_training은 아직 `tools/` 계층을 거치지 않고 각 scenario execute/response node에서 deterministic response를 반환합니다.
...
| deterministic execute/response node | exploration, quest, trade, dialogue, skill_training | lightweight sample 로직으로 response를 만든다. 아직 usecase/tool/payload persistence를 갖지 않는다. |
