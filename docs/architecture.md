...
현재 실제 tool-backed 시나리오는 battle과 craft뿐입니다. exploration, quest, trade, dialogue, skill_training은 아직 `tools/` 계층을 거치지 않고 각 scenario execute/response node에서 deterministic response를 반환합니다.
...
다른 시나리오는 아직 이 runner를 사용하지 않습니다. 이 경계는 의도적으로 좁게 유지되어 있습니다. scenario flow와 graph shape 일반화가 먼저이며, tool/usecase 일반화는 다음 확장 지점입니다.
...
| 종류 | 대상 | 특징 |
| --- | --- | --- |
| tool-backed execute node | battle, craft | `engine/tool_runner.py`를 통해 LangChain tool과 application usecase를 호출하고 payload를 store에 저장한다. |
